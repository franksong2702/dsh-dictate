...
- 可直接从插件设置页启动、停止、检查本地服务，并从当前浏览器测试端点连接与 CORS。
...
- “转写方式”默认为“浏览器语音识别”。选择“本地服务端点（实验性）”后，可配置一个 `localhost`、`127.0.0.1` 或 `[::1]` 回环地址；插件拒绝把麦克风录音发送到其他主机。设置页的“测试连接”会从当前浏览器访问 `/health`，同时验证服务可达性与 CORS 配置。
...
安装支持 `funasr-server` 的 FunASR 环境后，可以从插件设置页启动、停止并检查服务状态。当前上游 SenseVoice 模型文件接近 1 GB；首次使用可能需要先下载模型，随后还要加载到内存，实际耗时取决于网络、磁盘和 CPU。设置页会如实区分“检查运行环境、启动进程、检查模型文件、下载模型、加载模型、检查服务、已就绪”等阶段；只有上游输出真实下载百分比时才显示进度条，同时提供取消启动、失败后重试和诊断摘要。

取消启动会终止插件管理的服务进程；再次启动时会重新检查本地模型缓存，但缓存能否从中断处继续由上游 ModelScope 下载器决定。DSH host 必须能在 `PATH` 中找到 `funasr-server`，也可以用服务端环境变量明确指定路径和工作目录：
...
然后在“设置 → 插件 → 插件配置 → 上下文语音输入”中选择“本地服务端点（实验性）”，服务地址保留默认的 `http://127.0.0.1:39081`。设置页会区分插件管理的进程与外部服务；外部服务可以检查但不能由插件停止。可以使用“测试连接”确认当前浏览器确实能够访问端点。服务未启动、CORS origin 不匹配或返回异常时，界面会显示可读错误；Web Speech 默认选项不受影响。
...
- 本地端点是停止录音后的最终转写，目前提供 VAD 语音确认和约 600 ms 尾音保护，但不提供实时临时文字；服务进程、SenseVoice 模型和 CORS 白名单由用户在本机单独管理。
