...
- **用户确认的自定义词典**：当用户把短技术词纠正为 `Codex Connect` 等写法时，完成状态收起后可选择“记住”或“忽略”；插件不会自动保存整句修改。
...
6. 如果插件从一次短技术词修改中找到候选词，会在完成提示收起后询问是否记住；只有点击“记住”才写入当前浏览器的自定义词典。
...
- 自定义词典与上述临时词汇分开保存。用户可在“上下文增强 → 自定义词典”中手动添加或删除；词典保存在当前浏览器的 DSH 配置中，并优先用于支持短语提示的 Web Speech 和后续模型润色。本地 SenseVoice 端点当前不直接接收词典。
