...
Windows x64 local SenseVoice support is present in `main` and is planned for the next `dsh-dictate` alpha release. It is **not** included in the currently published npm `0.4.0-alpha.7` package.
...
- A future `dsh-dictate` alpha package that includes the Windows runtime.
...
1. Install the next published alpha into the same Web profile that DSH uses. Do not install `0.4.0-alpha.7` expecting Windows local ASR; that published artifact predates Windows support.
...
- **Local option missing:** confirm the installed Dictate package is the Windows-capable alpha, not npm `0.4.0-alpha.7`.
