...
Windows x64 local SenseVoice support is included in the `dsh-dictate@0.4.0-alpha.8` release candidate. The protected release workflow publishes the candidate to npm `next`; merging the source does not publish it. It is **not** included in npm `latest`, which remains `0.4.0-alpha.7` until a separate promotion decision.
...
- `dsh-dictate@0.4.0-alpha.8`, after npm `next` resolves to that version.
...
1. Confirm `npm view dsh-dictate dist-tags.next` returns `0.4.0-alpha.8`, then install `dsh-dictate@next` into the same Web profile that DSH uses. Do not install `0.4.0-alpha.7` expecting Windows local ASR; that artifact predates Windows support.
...
- **Local option missing:** confirm the installed Dictate package is `0.4.0-alpha.8`, not npm `0.4.0-alpha.7`.
