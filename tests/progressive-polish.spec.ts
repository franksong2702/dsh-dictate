...
describe('progressive transcript polish', () => {
  ...
  it('uses elapsed quiet time while coalescing updates behind a slow request', async () => {
    vi.setSystemTime(0)
    const pending = deferred<string>()
    const polish = vi.fn().mockReturnValueOnce(pending.promise).mockResolvedValue(cleaned)
    const run = new ProgressivePolish({ polish, preview: vi.fn(), join, intervalMs: 4000 })
    run.update(raw, '')
    await vi.advanceTimersByTimeAsync(350)
    run.update(raw, '先补文档')
    await vi.advanceTimersByTimeAsync(1000)
    run.update(raw, '不对最后改成补测试')
    await vi.advanceTimersByTimeAsync(4000)
    expect(polish).toHaveBeenCalledTimes(1)
    pending.resolve(cleaned)
    await vi.advanceTimersByTimeAsync(1)
    expect(polish.mock.calls.map(call => call[0])).toEqual([raw, `${raw}不对最后改成补测试`])
    expect(await run.finish(`${raw}不对最后改成补测试`)).toBe(cleaned)
    expect(run.metrics.finalCalls).toBe(0)
  })

  it('waits only the remaining quiet interval when speech changes just before completion', async () => {
    vi.setSystemTime(0)
    const pending = deferred<string>()
    const polish = vi.fn().mockReturnValueOnce(pending.promise).mockResolvedValue(cleaned)
    const run = new ProgressivePolish({ polish, preview: vi.fn(), join })
  ...
  it('starts the latest full snapshot at the two-second spacing boundary without adding a call at stop', async () => {
    vi.setSystemTime(0)
    const polish = vi.fn((text: string) => new Promise<string>(resolve => setTimeout(() => resolve(text), 800)))
    const run = new ProgressivePolish({ polish, preview: vi.fn(), join })
    run.update(raw, '')
    await vi.advanceTimersByTimeAsync(1500)
    const finalRaw = `${raw}不对第二条取消`
    run.update(finalRaw, '')
    await vi.advanceTimersByTimeAsync(849)
    expect(polish).toHaveBeenCalledTimes(1)
    await vi.advanceTimersByTimeAsync(1)
    expect(polish.mock.calls.map(call => call[0])).toEqual([raw, finalRaw])
    await vi.advanceTimersByTimeAsync(850)
    run.prepareToStop()
    expect(await run.finish(finalRaw)).toBe(finalRaw)
    expect(polish).toHaveBeenCalledTimes(2)
    expect(run.metrics.reused).toBe('completed')
    expect(run.metrics.stopCalls).toBe(0)
  })
